...
// Set all forces based in a sine wave
void FlowField::setSineWave() {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            float sineWave = ofMap( y, 0, flowList.size(), 0, 3*TWO_PI);
            flowList[y][x].set( ofVec2f( 1, sin(sineWave) ) );
        }
    }
}

// Set all forces based in a sine+cosine field
void FlowField::setSineCosineField() {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            float sineWave = ofMap( y, 0, flowList.size(), 0, 4*TWO_PI);
            float cosineWave = ofMap( x, 0, flowList.size(), 0, 3*TWO_PI);
            flowList[y][x].set( ofVec2f( cos (cosineWave), sin(sineWave) ) );
        }
    }
}
