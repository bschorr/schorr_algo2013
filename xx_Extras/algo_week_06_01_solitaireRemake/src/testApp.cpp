...
void testApp::update(){
    ...
    //bkg.grabScreen(0, 0, ofGetWindowWidth(), ofGetWindowHeight());
}

//--------------------------------------------------------------
void testApp::draw(){
    
    //bkg.draw(0, 0, ofGetWindowWidth(), ofGetWindowHeight());
    ...
    if (initCounter >= 0) {
        ...
        if (drawKings > 0) {
            for (int i = cards.size()-1; i > 0; i-=13) {
                cards[i].draw(realBack);
            }
            drawKings --;
        }
    
        cards[counter].draw(realBack);
    }
